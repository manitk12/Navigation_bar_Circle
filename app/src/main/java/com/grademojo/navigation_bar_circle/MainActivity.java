package com.grademojo.navigation_bar_circle;
...
public class MainActivity extends AppCompatActivity
    ...
    float Documents = 25f , Messages = 25f, Photos;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pieChart.setCenterText("ONLY 1600 FILES");
        ...
        Messages = new BigDecimal(Messages).setScale(DECIMAL_SCALE, BigDecimal.ROUND_FLOOR).floatValue();

        Photos = 100.0f - Documents - Messages;
        ...
        if(Photos > 0){
            entries.add(new PieEntry(Photos, "Photos"));
        }

        if(Documents > 0){
            entries.add(new PieEntry(Documents, "Documents"));
        }

        if(Messages > 0){
            entries.add(new PieEntry(Messages, "Messages"));
        }
        ...
        if(Photos == 0f){
            ...
            dataSet.setColors(ContextCompat.getColor(context.getContext(), R.color.dark_green),ContextCompat.getColor(context.getContext(), R.color.new_green), ContextCompat.getColor(context.getContext(), R.color.new_yellow));
        }
